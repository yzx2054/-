...
u8 PS_Sleep(void)//6.4.6 休眠指令  
{
  ...
  temp =  0x01+0x03+0x33;
	...
	data=JudgeStr(2000);
	if(data)
		  ensure=data[9];//00代表成功 
	else
		  ensure=0xff;
	return ensure;
}

u8 PS_Cancel(void)//6.4.5 取消指令 
{
	uint16_t temp;
  uint8_t  ensure;
	uint8_t  *data;
...
//u8 PS_AutoEnroll(u16 PageID,u8 *para)//自动注册指纹
//{
//	u16 temp;
//  u8  ensure;
//	u8  *data;
...
//	MYUSART_SendData(3);
//	MYUSART_SendData(0);
//	MYUSART_SendData(0x3C);	//参数 参见AS608文档
//	temp = 0x01+0x08+0x31
//	+(PageID>>8)+(u8)PageID
//	+3+0x3C;
//	SendCheck(temp);
...
//	return ensure;
//}
void PS_AutoEnroll(u16 PageID)//自动注册指纹  按N次注册
{
	u16 temp;	
	u8 input_time=4;//录入次数
	SendHead();
	SendAddr();
	SendFlag(0x01);//命令包标识
	SendLength(0x0008);
	...
	temp = 0x01+0x0008+0x31
	+(PageID>>8)+(u8)PageID
	+input_time+0x08;
	SendCheck(temp);
}
void PS_AutoIdentify(void)//自动验证指纹
{
		u16 temp;
...
//说明:  模块返回确认字+页码（相配指纹模板）
u8 PS_HighSpeedSearch(u8 BufferID,u16 StartPage,u16 PageNum,u8 *finger_n)
{
	u16 temp;
  u8  ensure;
	u8  *data;
...
u8 PS_ValidTempleteNum(u8 *ValidN)
{
	u16 temp;
  u8  ensure;
	u8  *data;
