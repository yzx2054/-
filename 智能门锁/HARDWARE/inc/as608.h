...
u8 PS_HighSpeedSearch(u8 BufferID,u16 StartPage,u16 PageNum,u8 *finger_number);//高速搜索 
  
u8 PS_ValidTempleteNum(u8 *ValidN);//读有效模板个数 

u8 PS_HandShake(u32 *PS_Addr); //与AS608模块握手
void PS_AutoIdentify(void);//自动验证指纹
const char *EnsureMessage(u8 ensure);//确认码错误信息解析
void PS_AutoEnroll(u16 PageID);//自动注册指纹
